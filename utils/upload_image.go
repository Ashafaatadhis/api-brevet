...
// UploadImageHandler fungsi untuk menghandling upload file
func UploadImageHandler(c *fiber.Ctx, formFile *multipart.FileHeader, path *string) (*string, error) {
...
// UploadImage fungsi untuk upload file yang siap pakai dibanding uploadFileHandler
func UploadImage(c *fiber.Ctx, fieldName string, path string) (*string, error) {
	...
	filePath, err := UploadImageHandler(c, file, &path)
	if err != nil {
		return nil, err
	}
